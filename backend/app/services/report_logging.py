"""Structured and console file logging for report generation."""
import json
import logging
import os
import threading
...
# Serializes handler attach/detach on shared loggers (mirofish.report_agent, etc.).
_LOGGER_HANDLER_LOCK = threading.Lock()
...
class ReportLogger:
    ...
    def log(
        ...
        # Append to JSONL file (thread-safe for parallel section generation)
...
class ReportConsoleLogger:
    ...
    # Mark FileHandlers we attach so a new instance can detach stale ones from shared loggers.
    _HANDLER_MARKER_ATTR = "_is_mirofish_report_console_handler"
    ...
    def _setup_file_handler(self):
        ...
        setattr(self._file_handler, self._HANDLER_MARKER_ATTR, True)
        ...
        with _LOGGER_HANDLER_LOCK:
            for logger_name in loggers_to_attach:
                target_logger = logging.getLogger(logger_name)
                for h in list(target_logger.handlers):
                    if getattr(h, self._HANDLER_MARKER_ATTR, False):
    ...
    def __enter__(self) -> "ReportConsoleLogger":
        return self
