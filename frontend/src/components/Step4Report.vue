            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content generation done, but entire section may not be complete) -->
...
import ReportOutlinePanel from './ReportOutlinePanel.vue'
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract sub-queries - full extraction, no limit
    const subQSection = text.match(/### Analyzed Sub-queries\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
    ...
    // Extract core entities - full extraction, includes summary and related fact count
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract currently valid facts - full extraction, no limit
    ...
    // Extract entities involved - full extraction, no limit
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Extract interviewee count (e.g. "5 / 9 simulated agents")
    ...
    // Extract interviewee selection reasoning
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*(?:选择|Select\s+)([^（(]+)(?:[（(]index\s*=?\s*\d+[)）])?[：:]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        // e.g.: - **Parent_601 (index 0)**: As a parent representative...
        if (!headerMatch) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extract bio (support both English and Chinese labels)
      ...
      if (qMatch) {
        ...
        // Split questions by numbered list
        const questions = qText.split(/\n\d+\.\s+/).filter(q => q.trim())
        if (questions.length > 0) {
          // Handle the first question if it starts with "1."
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Try to extract edge info (if available)
    ...
    // Try to extract node info (if available)
  ...
  setup(props) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'No key memory available'),
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'No core entities available'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'No relation chains available')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'No active memory available'),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'No historical memory available'),
        ...
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'No involved entities available'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // If no numbers found or only one found, return whole text
      if (matches.length <= 1) {
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform switch buttons (only shown when real dual-platform answers exist)
...
const activeStep = computed(() => {
  ...
  // If no active, return the last done step
...
// Polling for agent + console logs (short interval; avoids long-lived SSE holding a Flask worker)
let agentLogTimer = null
let consoleLogTimer = null

const applyAgentLogPayload = (res) => {
  ...
  agentLogLine.value = res.data.total_lines ?? res.data.from_line + newLogs.length
  nextTick(() => {
    if (rightPanel.value) {
      if (isComplete.value) {
...
const extractFinalContent = (response) => {
  ...
  // Try to find content after Chinese "Final Answer:" label
  const chineseFinalMatch = response.match(/最终答案[:：]\s*\n*([\s\S]*)$/i)
  if (chineseFinalMatch) {
    return chineseFinalMatch[1].trim()
  }
  
  // If starts with ## or # or >, likely direct markdown content
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // If content is long and contains markdown formatting, try to remove thinking process and return
...
const startPolling = () => {
  ...
  agentLogTimer = setInterval(fetchAgentLog, 1000)
  consoleLogTimer = setInterval(fetchConsoleLog, 1000)
}
