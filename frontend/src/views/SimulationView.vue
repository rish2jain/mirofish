 ...
 * When the user returns from Step 3 to Step 2, assume the user wants to exit the simulation
 */
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        // If graceful shutdown throws exception, try force stop
        await forceStopSimulation()
      }
    } else {
    ...
    // Failure to check environment status does not affect subsequent flow
...
const forceStopSimulation = async () => {
  try {
    ...
    addLog(`Exception force stopping simulation: ${err.message}`)
  }
}
