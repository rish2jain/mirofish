...
| **P3** | **SSE** `GET /api/simulation/<id>/run-status/stream`; **Step3** `EventSource` + fallback (`VITE_SIMULATION_SSE`); **report logs** JSON poll `GET /api/report/<id>/agent-log` + `console-log` (optional alias `.../agent-log/poll`, `.../console-log/poll`); **Step4** polls those endpoints (`VITE_REPORT_LOG_SSE` naming is legacy); MCP `pollSimulation` backoff; **GraphPanel:** incremental force simulation when structure is a strict superset (reuse positions), not full enter/update/exit | Strict D3 enter/update/exit pattern; graph build / prepare progress over SSE (Step2/MainView still mostly poll) |
...
2. Further split large frontend files, mainly [`frontend/src/components/Step2EnvSetup.vue`](../frontend/src/components/Step2EnvSetup.vue) and remaining heavy sections in [`frontend/src/components/Step4Report.vue`](../frontend/src/components/Step4Report.vue) and [`frontend/src/components/Step5Interaction.vue`](../frontend/src/components/Step5Interaction.vue).
...
| Report agent / console log push | `app/api/report.py`, `Step4Report.vue` | SSE for log tail | **HTTP poll:** **`GET .../agent-log`**, **`.../console-log`** (aliases **`.../agent-log/poll`**, **`.../console-log/poll`**) — JSON snapshots, not SSE |
...
|---------|-------|------------|---------------------|
| WebSocket/SSE for real-time updates | Replace polling with push for sim progress, report gen, agent logs | Medium | **Partial:** SSE for `run-status/stream`; report agent/console logs are **HTTP poll** (`agent-log` / `console-log` / `.../poll`); Step2/MainView graph/task still mostly HTTP poll; no WebSocket |
...
1. **SSE:** simulation **`/run-status/stream`** + Step3; report logs **HTTP poll** **`agent-log`** / **`console-log`** (aliases **`.../poll`**) + Step4; **`POST /api/report/chat/stream`** for narrative streaming
